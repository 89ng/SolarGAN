...
class DoppelGANger(object):
    def __init__(self, sess, checkpoint_dir, sample_dir, time_path,
                 ...
                 attribute_latent_dim=5, feature_latent_dim=10,
    ...
    def build_connection(self):
        ...
        #if add weather, change the dim here self.feature_latent_dim+self.ori_feature_dim*self.sample_len
        ...
        self.given_attribute_train_pl = []
        for i in range(self.num_packing):
            self.given_attribute_train_pl.append(
                tf.placeholder(
                    tf.float32,
                    [None, self.sample_real_attribute_dim],
                    name="given_attribute_{}".format(i))) 
            ...
            (g_output_feature_train_tf, g_output_attribute_train_tf,
             ...
             g_output_argmax_train_tf) = \
                self.generator.build(
                    ...
                    train=True,
                    attribute=self.given_attribute_train_pl[i])
    ...
    def sample_from(self, real_attribute_input_noise,
        ...
        for i in range(round_):
            if given_attribute is None:
                raise Exception("real_attributes_should_be_given!")
    ...
    def visualize(self, epoch_id, batch_id, global_id):
        def sub1(features, attributes, lengths,
            ...
            if ground_truth_features is None:
                ...
                draw_attribute(
                    np.concatenate([attributes[:,-2:], ground_truth_attributes[:,-2:]],
                                   axis=0),
                    self.data_attribute_outputs[-2:],
                    file_path)
        ...
        given_attribute_test = ground_truth_attribute[:,:-2]
        
        input_data = self.gen_feature_input_data_free(self.vis_num_sample)
        
        features, attributes, gen_flags, lengths = self.sample_from(
            None, addi_attribute_input_noise,
            feature_input_noise, input_data, given_attribute=given_attribute_test,
        ...
        for epoch_id in tqdm(range(self.epoch)):
            ...
            for batch_id in range(batch_num):
                ...
                for i in range(self.num_packing):
                    ...
                    batch_data_attribute_train = batch_data_attribute[:,:-2]
                    ...
                    feed_dict[self.given_attribute_train_pl[i]] = \
                        batch_data_attribute_train
                    feed_dict[self.
