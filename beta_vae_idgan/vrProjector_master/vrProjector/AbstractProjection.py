...
class AbstractProjection:
  ...
  @staticmethod
  def _readImageArray(imarray):

    imarray = imarray.astype(int)
    npimage = imarray.reshape(imarray.shape[1], imarray.shape[0], 3)
    imsize = npimage.shape
    return npimage, imsize

  def readImageArray(self, imageArray):
    self.image, self.imsize = self._readImageArray(imageArray)
    self.set_angular_resolution()
  ...
  @staticmethod
  def _outputImage(img, imgsize,dest):
        
    img_resized = cv2.resize(img, imgsize, interpolation = cv2.INTER_AREA)
    
    return img_resized

  def outputImage(self,dest):
    img = self._outputImage(self.image, self.imsize,dest)
    return img
