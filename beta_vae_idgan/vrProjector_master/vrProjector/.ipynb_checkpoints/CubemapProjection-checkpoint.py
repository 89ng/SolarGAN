...
class CubemapProjection(AbstractProjection):
  ...
  def readImageArrays(self, front, right, back, left, top, bottom):
    self.front, self.imsize = self._readImageArray(front)
    self.right, self.imsize = self._readImageArray(right)
    self.back, self.imsize = self._readImageArray(back)
    self.left, self.imsize = self._readImageArray(left)
    self.top, self.imsize = self._readImageArray(top)
    self.bottom, self.imsize = self._readImageArray(bottom)
    self.set_angular_resolution()
