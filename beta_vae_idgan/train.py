...
if __name__ == "__main__":
    ...
    # Save for tests
    ntest = batch_size
    x_real = utils.get_nsamples(train_loader, ntest)
    ...
    while not out:
        ...
        for x_real in train_loader:
            ...
            if (it % config['training']['sample_every']) == 0:
                ...
                x_real_im = x_real.add(1).div(2)
                
                if x_real_im.size(1) == 5:
                    x_real_im = x_real_im.add(1).div(2).clamp(0, 1).round().argmax(dim=1).unsqueeze(1).cpu()/4
                else:
                    x_real_im = x_real_im
                    
                x_real_im = make_grid(x_real_im, nrow=int(sqrt(batch_size)), padding=2, pad_value=1)
                
                logger.add_imgs(x_real_im, 'input_images', it)
